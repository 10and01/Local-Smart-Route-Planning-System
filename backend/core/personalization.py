...
{
  "theme_weights": {"关键词1": 0.0~1.0, "关键词2": ..., ...},
...
提取规则：
1. theme_weights 自由提取用户描述中的偏好关键词，不限数量、不限类型。
   - 用户明确喜欢的关键词（"我喜欢美食和拍照"）给 0.8~0.95
   - 用户明确不喜欢的关键词（"我不爱购物"）给 0.05~0.15
   - 用户没明确提及但与描述相关的关键词给 0.3~0.5
   - 无关的关键词不列出
   - 示例："我喜欢吃辣、拍照，讨厌排队" → {"吃辣": 0.9, "拍照": 0.85, "排队": 0.1}
   - 示例："想带孩子去游乐园和博物馆" → {"亲子": 0.95, "游乐园": 0.9, "博物馆": 0.85, "儿童": 0.8}
2. traveler_type：从描述中推断最常出行的群体
3. pace_preference：用户平时的旅行节奏倾向
4. budget_level：用户一贯的消费水平
5. price_sensitivity：用户对价格的敏感程度
6. willingness_to_queue：用户是否愿意排队
...
请严格按以下 JSON 格式输出（只输出纯 JSON，不要任何解释）：
{
  ...
  "delta": {
    "theme_weights": {"新增或变化的关键词": 0.0~1.0},
...
2. theme_weights 只包含**变化**的关键词，不需要列出所有关键词。
3. 数值含义：
   - 0.8~0.95 = 用户明确喜欢/强调
...
class UserPersonalizationEngine:
    ...
    def profile_to_preference(self, profile: Optional[Dict[str, Any]]) -> Optional[UserPreference]:
        ...
        # 【重构】不再强制要求6个固定主题，直接使用数据库中的自由关键词
    ...
    def evolve_profile(
        ...
        if selected_plan_theme:
            theme_boost_map = {
                "深度体验": {"深度体验": 0.15, "探索": 0.05},
                "高效省时": {"高效": 0.15, "紧凑": 0.05},
                "均衡推荐": {"均衡": 0.1},
            }
    ...
    def _parsed_to_user_preference(parsed: dict) -> UserPreference:
        ...
        if isinstance(llm_themes, dict):
            for kw, val in llm_themes.items():
                try:
                    theme_weights[kw] = max(0.0, min(1.0, float(val)))
